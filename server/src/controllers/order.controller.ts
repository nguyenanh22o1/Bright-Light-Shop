...
import { checkDuplicateItem, checkItemQuantityMoreThanAvailableNumber, createOrder, findOrder, findOrders, getOrderDetailsWithItem, getOrderResponse, processOrder, returnOrder, saveRewardPoint } from "../services/order.service";
...
export async function createOrderHandler(
  ...
  try {
    ...
    const isItemQuantityMoreThanAvailableNumber =
      await checkItemQuantityMoreThanAvailableNumber(orderDetailsWithItem);
...
export async function getAllOrdersHandler(req: Request, res: Response) {
  try {
    ...
    return res
      .status(200)
      .json(paginatorUtil.paginate(ordersResponse, page, 5));
  } catch (error: any) {
...
export async function returnOrderHandler(
  ...
  try {
    ...
    if (!isOntime) {
      const balance = ((await findUser({ _id: userId })) as UserDocument)
        .balance;
